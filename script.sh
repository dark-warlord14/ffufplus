		...
		"Directory BruteForcing")
			...
			printf "\nDone. Result is stored in result_wayback.txt\n"
                ...
                "Beast Mode")
                        ...
                        cat ffuf/* | jq '[.results[]|{status: .status, length: .length, url: .url}]' | grep -oP "status\":\s(\d{3})|length\":\s(\d{1,7})|url\":\s\"(http[s]?:\/\/.*?)\"" | paste -d' ' - - - | awk '{print $2" "$4" "$6}' | sed 's/\"//g' | sort -u > result_beast.txt
                        rm ffuf -r
