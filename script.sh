		...
		"Directory BruteForcing")
			...
			printf "\nDone. Result is stored in result_wayback.txt\n"
                ...
                "Beast Mode")
                        echo "Beast Mode ON"
                        mkdir ffuf
                        xargs -P `nproc` -I {} sh -c 'url="{}"; ffuf -s -mc all -H "X-Forwarded-For: 127.0.0.1" -H "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:72.0) Gecko/20100101 Firefox/72.0" -u "{}/FUZZ" -w wordlist/dicc.txt -t 50 -D -e js,php,bak,txt,asp,aspx,jsp,html,zip,jar,sql,json,old,gz,shtml,log,swp,yaml,yml,config,save,rsa,ppk -ac -se -o ffuf/${url##*/}-${url%%:*}.json' < alive.txt
